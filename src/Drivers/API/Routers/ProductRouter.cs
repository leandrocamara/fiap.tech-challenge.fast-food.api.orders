...
public class ProductRouter(IProductController controller) : BaseRouter
{
    ...
    {
        var result = await controller.DeleteProduct(request);
        return HttpResponse(result);
    }
    ...
    [SwaggerResponse(StatusCodes.Status500InternalServerError, "Erros não tratados pelo sistema, sendo retornado o erro específico no corpo da resposta.")]
    public async Task<IActionResult> GetProductsByCategory([FromQuery] GetProductsByCategoryRequest request)
    {
        var result = await controller.GetProductsByCategory(request);
        return HttpResponse(result);
    }
    ...
    [SwaggerResponse(StatusCodes.Status500InternalServerError, "Erros não tratados pelo sistema, sendo retornado o erro específico no corpo da resposta.")]
    public async Task<IActionResult> GetProductsByIdResponse([FromQuery] GetProductByIdRequest request)
    {
        var result = await controller.GetProductById(request);
        return HttpResponse(result);
    }
}